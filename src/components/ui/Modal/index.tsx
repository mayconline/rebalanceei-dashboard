...
function DialogContent({
  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <Content
        className={mergeClass(
          'data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border bg-background-secondary p-6 shadow-lg duration-200 data-[state=closed]:animate-out data-[state=open]:animate-in sm:max-w-lg',
          className
        )}
...
function DialogTitle({ className, ...props }: ComponentProps<typeof Title>) {
  return (
    <Title
      className={mergeClass('font-semibold text-lg leading-none', className)}
